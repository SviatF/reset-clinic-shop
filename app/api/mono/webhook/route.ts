...
async function decrementStock(order: OrderRecord) {
  ...
  await mutateProducts(`Order ${order.order_number}: decrement stock`, (products) => products.map((product) => {
    ...
    return { ...product, stock_quantity: Math.max(0, Number(product.stock_quantity || 0) - Number(line.quantity || 0)), updated_at: new Date().toISOString() };
  }));
  ...
  for (const line of order.items) await appendActivity({ event_type: "stock_decremented", entity_type: "product", entity_id: line.product_id, title: `Stock списано: ${line.name} · ${line.quantity} шт.`, metadata: { orderId: order.id, quantity: line.quantity } });
}

export async function POST(request: Request) {
  const rawBody = await request.text();
  const signature = request.headers.get("x-sign") || "";
  if (!(await verifyMonoWebhook(rawBody, signature))) return NextResponse.json({ error: "Invalid mono webhook signature" }, { status: 401 });
  try {
    const payload = JSON.parse(rawBody) as { invoiceId?: string; status?: string; amount?: number; finalAmount?: number; modifiedDate?: string; reference?: string; paymentInfo?: unknown };
    ...
    const nextOrders = await mutateOrders(`Payment ${before.order_number}: ${payload.status}`, (orders) => orders.map((order) => order.id !== before.id ? order : {
      ...order,
    ...
    const updated = nextOrders.find((item) => item.id === before.id);
